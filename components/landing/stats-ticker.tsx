        ...
        svg: (
            <svg viewBox="0 0 280 140" fill="none" className="w-full">
                ...
                <g className="pe1">
                    ...
                    <text x="16" y="26" fontSize="8" fill="currentColor" fillOpacity="0.4" fontFamily="inherit">Figma deck (static)</text>
                    <text x="16" y="44" fontSize="10" fill="#c2553f" fillOpacity="0.7" fontFamily="inherit">mockup.fig</text>
                    <text x="16" y="58" fontSize="10" fill="#c2553f" fillOpacity="0.7" fontFamily="inherit">never deployed...</text>
                </g>
                <g className="pe2">
                ...
                <g className="pe3">
                    ...
                    <text x="172" y="26" fontSize="8" fill="currentColor" fillOpacity="0.4" fontFamily="inherit">Shunya in production</text>
                    <text x="172" y="42" fontSize="9" fill="currentColor" fillOpacity="0.7" fontFamily="inherit">app.live ✓ 200 OK</text>
                    <text x="172" y="58" fontSize="9" fill="currentColor" fillOpacity="0.5" fontFamily="inherit">Uptime: 99.9%</text>
                </g>
                <text x="8" y="96" fontSize="10" fill="currentColor" fillOpacity="0.3" fontFamily="inherit">Concept to production -</text>
                <text x="8" y="110" fontSize="10" fill="currentColor" fillOpacity="0.3" fontFamily="inherit">software that actually gets used</text>
            </svg>
        ),
    },
    {
                ...
                <style>{`.fa1{animation:fa-in 0.4s ease both;animation-play-state:var(--anim-play,paused);animation-delay:0.1s}.fa2{animation:fa-in 0.4s ease both;animation-play-state:var(--anim-play,paused);animation-delay:0.3s}.fa3{animation:fa-in 0.4s ease both;animation-play-state:var(--anim-play,paused);animation-delay:0.5s}.fa4{animation:fa-in 0.4s ease both;animation-play-state:var(--anim-play,paused);animation-delay:0.7s}.fa5{animation:fa-in 0.4s ease both;animation-play-state:var(--anim-play,paused);animation-delay:0.9s}@keyframes fa-in{from{opacity:0;transform:translateX(-8px)}to{opacity:1;transform:translateX(0)}}`}</style>
                {
                    [
                        ...
                        <g key={i} className={`fa${i + 1}`}>
                            ...
                            <text x="24" y={row.y + 12} fontSize="9.5" fill="currentColor" fillOpacity={row.highlight ? 0.9 : 0.5} fontFamily="inherit">{row.label}</text>
                            <text x="160" y={row.y + 12} fontSize="9.5" fill="currentColor" fillOpacity={row.highlight ? 0.9 : 0.35} fontFamily="inherit">{row.note}</text>
                        </g>
                    ))
                }
            </svg>
                ...
                <style>{`.ca1{animation:ca-pop 0.4s cubic-bezier(0.22,1,0.36,1) both;animation-play-state:var(--anim-play,paused)}.ca2{animation:ca-pop 0.4s cubic-bezier(0.22,1,0.36,1) both;animation-play-state:var(--anim-play,paused);animation-delay:0.15s}.ca3{animation:ca-pop 0.4s cubic-bezier(0.22,1,0.36,1) both;animation-play-state:var(--anim-play,paused);animation-delay:0.3s}.ca4{animation:ca-pop 0.4s cubic-bezier(0.22,1,0.36,1) both;animation-play-state:var(--anim-play,paused);animation-delay:0.45s}.ca5{animation:ca-pop 0.4s cubic-bezier(0.22,1,0.36,1) both;animation-play-state:var(--anim-play,paused);animation-delay:0.6s}.ca6{animation:ca-pop 0.4s cubic-bezier(0.22,1,0.36,1) both;animation-play-state:var(--anim-play,paused);animation-delay:0.75s}@keyframes ca-pop{from{opacity:0;transform:scale(0.8)}to{opacity:1;transform:scale(1)}}`}</style>
                {
                    [
                        ...
                        <g key={label} className={cls}>
                            ...
                            <text x={x + 4} y={y + 14} textAnchor="middle" fontSize="9" fill="currentColor" fillOpacity="0.5" fontFamily="inherit">{label}</text>
                        </g>
                    ))
                }
                <text x="140" y="125" textAnchor="middle" fontSize="10" fill="currentColor" fillOpacity="0.3" fontFamily="inherit">Modern stacks - fast, with zero debt</text>
            </svg>
        ),
    },
]
...
/**
 * Each card's SVG animates with `animation-play-state: var(--anim-play, paused)` and fill-mode
 * `both`, so while the variable stays `paused` the shapes hold their `from` keyframe - which is
 * opacity 0. Scrolling the card into view has to be what flips it to `running`; hover is only kept
 * as a belt-and-braces trigger.
 */
function PointCard({ point }: { point: (typeof POINTS)[number] }) {
    ...
    return (
        ...
        >
            <span className="font-mono text-[11px] font-semibold tracking-[0.16em] text-neutral-400 dark:text-neutral-600 mb-4">{point.n}</span>
            <div className="mb-5 opacity-80">{point.svg}</div>
            <h3 className="text-[17px] font-bold mb-2 leading-snug">{point.title}</h3>
            <p className="text-[13.5px] text-neutral-500 dark:text-neutral-400 leading-relaxed">{point.body}</p>
        </motion.div>
    )
}
    ...
    return (
        <section className="relative py-24 md:py-32 overflow-hidden">
            ...
            <div className="mx-auto max-w-6xl px-6">
                ...
                >
                    {POINTS.map((p) => <PointCard key={p.n} point={p} />)}
                </motion.div>
                <motion.div
                ...
                >
                    {/* --tw-gradient-from is never set here, so the old fallback painted a white
                        veil over the marquee in dark mode. The page background token is the colour
                        the edges actually have to fade into. */}
                    <div aria-hidden className="absolute inset-y-0 left-0 w-24 z-10 pointer-events-none"
                        style={{ background: "linear-gradient(to right, var(--so-bg), transparent)" }} />
                    <div aria-hidden className="absolute inset-y-0 right-0 w-24 z-10 pointer-events-none"
                        style={{ background: "linear-gradient(to left, var(--so-bg), transparent)" }} />
                    <Marquee />
                    <Marquee reverse />
                </motion.div>
            </div>
